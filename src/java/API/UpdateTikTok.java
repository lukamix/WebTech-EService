...
@WebServlet(name = "UpdateTikTok", urlPatterns = {"/update-tiktok"})
public class UpdateTikTok extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        request.setCharacterEncoding("UTF-8");
        PrintWriter out = response.getWriter();
        int tiktokid = Integer.parseInt(request.getParameter("tiktokid").toString());
        ...
        try{
            ...
            if(CheckRole.getInstance().byUserNameAndPassword(Cookie.get(0),Cookie.get(1))){
                ...
                ts.update(tiktokid,html);
                out.println("<script type=\"text/javascript\">");
                out.println("alert('Cập Nhật Thành Công !');");
                out.println("window.location.href='admin?tiktokpage=1';");
                out.println("</script>");
            }
            else{
